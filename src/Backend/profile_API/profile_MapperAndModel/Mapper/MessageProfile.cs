...
public class MessageProfile: AutoMapper.Profile
{
    public MessageProfile()
    {
        ...
        CreateMap<MessageEntity, BaseMessage>().ForMember(x=>x.Username, opt=>opt.MapFrom(x=>x.UserEntity.Login)).ReverseMap();
        CreateMap<MessageEntity, CreateMessage>().ForMember(x=>x.ChatId, opt=>opt.MapFrom(x=>x.ChatId))
            .ForMember(x=>x.UserId, opt=>opt.MapFrom(x=>x.UserId)).ReverseMap();
    }
}